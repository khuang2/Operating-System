/*********************************************************************
 ...
 4.20 Jan        2015: Make thread safe.  Implement multiprocessors
 ************************************************************************/
...
void MemoryMappedIO(INT32 address, MEMORY_MAPPED_IO *mmio, BOOL ReadOrWrite) {
	...
	case Z502Disk: {
		mmio->Field4 = 0;
		if (mmio->Mode == Z502Status) {
			if (mmio->Field1 >= 1 && mmio->Field1 <= MAX_NUMBER_OF_DISKS) {
		...
		if ((mmio->Field1 >= 1 && mmio->Field1 <= MAX_NUMBER_OF_DISKS )
				&& (mmio->Field2 >= 0 && mmio->Field2 <= NUM_LOGICAL_SECTORS )) {
...
void MakeContext(long *ReturningContextPointer, long starting_address,
	...
	PageTable[VIRTUAL_MEM_PAGES - 1] = Temporary;
...
void SwitchContext(void **IncomingContextPointer, BOOL DoStartSuspend) {
    ...
    HardwareStats.TimeWeightedNumberRunningProcesses += HardwareStats.NumberRunningProcesses;
...
void HardwareInterrupt(void) {
	...
	while (TRUE ) {
		...
		if (event_type >= DISK_INTERRUPT
			...
			if (DO_DEVICE_DEBUG) {
				DataPointer = (INT32 *) DiskState[event_type - DISK_INTERRUPT
						+ 1].Source;
				printf(
...
void PrintHardwareStats(void) {
	...
	for (i = 1; i < MAX_NUMBER_OF_DISKS + 1; i++) {
	...
	MeanNumberRunningProcesses = (double)HardwareStats.TimeWeightedNumberRunningProcesses/(double)HardwareStats.context_switches;
	...
	if (HardwareStats.context_switches > 0)
		printf("Context Switches = %5d:  Mean Number Running Threads = %6.2f   ", HardwareStats.context_switches,
